...
function uniqueViolation(error: PostgresError) {
	...
	return new RecordNotUniqueError(collection, field);
}
...
function numericValueOutOfRange(error: PostgresError) {
	...
	return new ValueOutOfRangeError(collection, field);
}
...
function valueLimitViolation(error: PostgresError) {
	...
	return new ValueTooLongError(collection, field);
}
...
function notNullViolation(error: PostgresError) {
	...
	if (error.message.endsWith('contains null values')) {
		return new ContainsNullValuesError(table, column);
	}

	return new NotNullViolationError(table,column);
}
...
function foreignKeyViolation(error: PostgresError) {
	...
	return new InvalidForeignKeyError(collection,field);
}
